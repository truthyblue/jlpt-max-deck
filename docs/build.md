...
[v1.0.0 릴리스 노트](releases/v1.0.0.md)
...
- 같은 v1.0.0 GitHub Release의 빌더 코드와 아래 네 asset
...
Linux에서는 완성 APKG를 Anki Desktop으로 가져와 공부할 수 있습니다. 하지만
v1.0.0에서 PDF 17개를 처리하여 APKG를 만드는 빌드 대상은 아닙니다.
...
소스 코드 태그와 bundle ZIP, checksum, release pin, 배포 manifest는 반드시
[같은 v1.0.0 Release](https://github.com/truthyblue/jlpt-max-deck/releases/tag/v1.0.0)에서
...
| --- | --- |
| 릴리스 태그 | `v1.0.0` |
| ZIP 크기 | `849801445` bytes |
| ZIP SHA-256 | `458ffad3ab707983aacba5d79fa80d61069bdc9abcaa6916d656720c4cd8e9ea` |
| bundle manifest SHA-256 | `234c0eae5902af898d9eea26856b8c037d2fdbe46ffe9644312171fe16bb29ff` |
| public builder source hash | `afe40ff3b2e429964bea3c073ccace742555afd329fcebe91832c26a874d9fae` |
| logical APKG hash | `0ada593a07b5ab5a02ea1261b4fa1d58efb706f49323a02146558a8a1ce7e6ff` |

### macOS

네 파일을 같은 폴더에 둔 다음 실행합니다. 검증 명령은 ZIP, checksum,
`public-release.json`을 사용하며 `public-distribution-manifest.json`은 배포 전체의
수치를 사람이 확인할 때 사용합니다.
...
웹 시작 가이드의 명령은 v1.0.0에 고정된 시작 스크립트를 내려받아
실행합니다. 빌드가 성공하면 결과는 다음 폴더에 남습니다.

- macOS: `$HOME/JLPT-MAX-public-release-v1.0.0`
- Windows: `%USERPROFILE%\JLPT-MAX-public-release-v1.0.0`

시작 스크립트는 다운로드, 압축 해제와 Python 전용 환경에 쓴 임시 작업 폴더를
성공해도, 실패해도 자동으로 지웁니다. 위의 안정적인 결과 폴더는 지우지
않습니다. 성공한 뒤 결과 폴더를 Finder 또는 파일 탐색기로 열어 줍니다.
자동으로 열지 못하면 마지막에 출력된 경로를 직접 여세요.

아래 명령은 ZIP을 직접 풀고 실행하는 기술적 방법입니다. 이 방법은 결과
폴더를 자동으로 열지 않습니다.
...
APKG를 압축 해제하거나 결과 폴더 전체를 Anki에서 선택하지 마세요. 웹 시작
가이드의 한 줄 명령으로 실행했을 때만 결과 폴더를 Finder 또는 파일 탐색기로
자동으로 엽니다. ZIP 안의 `build-public` 스크립트를 직접 실행하면 사용자가
지정한 경로에 결과만 만들고 폴더를 자동으로 열지 않습니다.
...
- `expected_logical_apkg_hash`가
  `0ada593a07b5ab5a02ea1261b4fa1d58efb706f49323a02146558a8a1ce7e6ff`
...
PDF 17개·785쪽을 쓴 전체 빌드는 macOS arm64에서 실제로 수행했습니다. 결과는
`passed`, `unresolved: 0`이고 APKG SHA-256은
`7add6df097b7d00f92f52d66dc7c43ff76520e37242bbe4178cc8710b99db958`입니다.

Windows x64에서는 공개 자료로 실행하는 자동 테스트와 PowerShell 스크립트 검사는
통과했습니다. 하지만 PDF 17개를 쓴 전체 빌드는 아직 수행하지 않았습니다.
따라서 v1.0.0은 Windows 전체 빌드가 실제로 완료됐다는 증거로 쓰면 안 됩니다.
세부 검증값은 [v1.0.0 릴리스 노트](releases/v1.0.0.md)에 있습니다.

## 5. Anki에 가져오기

Anki가 처음이라면 [Anki 설치와 기기별 가져오기](anki.md)를 먼저 확인하세요.
완성 APKG는 Windows·macOS·Linux Desktop, iPhone/iPad, Android에 각각 직접
가져올 수 있습니다.
...
새 릴리스로 업데이트할 때는 소스 코드 태그, bundle ZIP, checksum,
`public-release.json`, `public-distribution-manifest.json` 다섯 항목을 모두 같은
릴리스로 교체하세요. 이전 bundle 위에 새 파일을 덮어쓰지 말고 새 디렉터리에
압축을 푸는 것이 안전합니다.
