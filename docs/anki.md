...
macOS 12 이상 또는 Windows x64 컴퓨터에서 하지만, 완성된
<code>JLPT-MAX덱-1.0.0.apkg</code>는 Windows, macOS, Linux, iPhone/iPad,
Android에 각각 직접 가져올 수 있습니다. Linux는 공부할 덱을 가져오는 것만
지원하며 PDF 17개로 덱을 만드는 빌드 대상은 아닙니다.
...
| --- | --- | --- | --- |
| Windows·macOS·Linux | Anki Desktop | 무료 | 파일 → 가져오기 |
...
## 2. Windows·macOS·Linux에 Anki Desktop 설치
...
1. [Anki 공식 다운로드 페이지](https://apps.ankiweb.net/)에서 Linux용 Anki Desktop을
   설치합니다.
2. macOS 12 이상 또는 Windows x64에서 만든 APKG를 Linux 컴퓨터로
   옮깁니다.
3. Anki의 파일(File) → 가져오기(Import)에서 APKG를 선택합니다.

Linux용 Anki에 APKG를 넣어 공부하는 것은 가능하지만, 이 프로젝트는 Linux에서의
PDF 17개 전체 빌드를 지원하지 않습니다.
...
5. 처음에도 Optimize를 눌러 확인할 수 있습니다. 최신 Anki는 리뷰 수가 적어도
   Optimize를 사용할 수 있습니다. 기록이 적으면 현재 값이 이미 최적이라는 안내가
   나올 수 있습니다. 이후에는 한 달에 한 번 정도면 충분합니다.
...
[AnkiDroid 덱 옵션](https://docs.ankidroid.org/manual.html#_other_deck_actions)을
확인하세요.
...
어휘 카드의 답을 열면 기본으로 단어 음성 하나가 재생됩니다. 답 오른쪽 위의
`재생` 메뉴를 누르면 다음을 고를 수 있습니다.

- `예문 자동재생` 끄기: 단어만 재생
- `첫 예문`: 단어가 끝나면 첫 예문까지 재생
- `모든 예문`: 단어가 끝나면 카드에 있는 예문을 순서대로 재생

재생 중에 단어나 예문을 직접 누르면 남은 자동재생은 멈추고, 누른 음성을
처음부터 들려줍니다. 이 설정은 해당 기기에 저장되며 AnkiWeb으로 다른 기기에
따라가지 않습니다. 여러 기기를 쓰면 기기마다 한 번씩 골라 주세요.

### 덱 안내·업데이트 링크

JLPT MAX덱 루트 덱의 설명과 모든 카드 답 아래에 `덱 안내 · 업데이트`
링크가 있습니다. 누르면 사용법, 업데이트와 오류 제보 페이지를 엽니다. 링크를
누르지 않으면 자동으로 인터넷에 연결하지 않습니다. 링크를 눌러도 PDF나
PDF에서 가져온 내용을 사이트로 보내지 않습니다.
