...
1. 소스 코드 태그가 `v1.0.0`인지, bundle ZIP, `.sha256`, `public-release.json`,
   `public-distribution-manifest.json`이 모두 같은 v1.0.0 GitHub Release에서
   왔는지 확인합니다.
...
v1.0.0 빌더는 macOS 12 이상 또는 Windows x64에서 실행합니다. Linux에서는
이 빌더로 PDF 17개를 처리하지 마세요. 지원되는 Mac 또는 Windows x64에서
APKG를 만든 다음 Linux용 Anki Desktop에 가져오는 것은 가능합니다.

v1.0.0의 PDF 17개 전체 빌드는 macOS에서 검증했습니다. Windows의 공개
자동 테스트는 통과했지만, Windows에서 PDF 17개를 쓴 전체 빌드는 아직
수행하지 않았습니다. Windows에서 문제가 나면 검증을 우회하지 말고 아래
‘안전한 issue 작성’ 범위의 정보만 제보해 주세요.
...
웹 시작 가이드의 한 줄 명령은 다운로드·압축 해제·전용 환경에 쓴 임시 폴더를
성공하거나 실패한 뒤 자동으로 지웁니다. 완성된 결과 폴더는 다음에 계속
남습니다.

- macOS: `$HOME/JLPT-MAX-public-release-v1.0.0`
- Windows: `%USERPROFILE%\JLPT-MAX-public-release-v1.0.0`

결과 폴더가 자동으로 열리지 않으면 실패가 아닐 수 있습니다. 터미널이나 PowerShell
마지막에 표시된 위 경로를 직접 열어 보세요. ZIP 안의 `build-public` 스크립트를
직접 실행한 경우에는 폴더 자동 열기를 하지 않습니다.
...
어휘 카드는 답을 열 때 단어를 기본으로 한 번 재생합니다. 답 오른쪽 위의
`재생` 메뉴에서 `예문 자동재생`을 켰다면 `첫 예문` 또는 `모든 예문`이
선택되었는지도 확인하세요. 설정은 기기별로 따로 저장되므로 다른 기기에서는
다시 골라야 합니다.
...
미디어 17,489개는 첫 동기화에 시간이 걸릴 수 있으므로 당장 공부할 기기에는
