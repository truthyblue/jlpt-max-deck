...
[README](../README.md) · [한자 덱 만들기](build.md) · [Anki 가이드](anki.md)
...
| 한자 읽기·쓰기 덱이 없음 | 정상 동작 또는 [한자 덱 만들기](build.md) |
| PDF·자동 준비·빌더 오류 | 아래 **한자 덱 빌더** |
...
- 파일: `JLPT-MAX-Deck-2.1.1.apkg`
- 크기: 약 1.15GB
- SHA-256: `4e4c212fada6e6af8c268aba70f289784486b1c6eee787e38707a23e43837627`

macOS:

```bash
shasum -a 256 JLPT-MAX-Deck-2.1.1.apkg
```

Windows PowerShell:

```powershell
Get-FileHash .\JLPT-MAX-Deck-2.1.1.apkg -Algorithm SHA256
```

같은 v2.1.1의
[SHA256SUMS](https://github.com/truthyblue/jlpt-max-deck/releases/download/v2.1.1/SHA256SUMS)와
결과가 다르면 파일을 삭제하고 안정적인 네트워크에서 다시 받으세요.

### 기본 덱에 한자 읽기·쓰기 덱이 없습니다

정상입니다. 기본 덱에는 문법·어휘·음성·실전 문제·참조표만 있습니다. 한자 읽기·쓰기 덱은
필요한 학습자만 [별도 APKG로 만듭니다](build.md).

### 예상 수량과 다릅니다

처음 설치한 기본 덱은 노트 20,650개, 카드
38,967개, 미디어 25,511개입니다.
...
iPhone·iPad는 v1.3.0부터 무음 모드에서도 카드 음성을 내는 방식을 기본으로
사용합니다. v1.2.1까지처럼 다른 앱 음악과 카드 음성을 섞어 재생하려면 답안
오른쪽 위 **설정 → 다른 앱 음악과 함께 재생**을 켜세요. 이 옵션을 켜면
iPhone·iPad의 무음 모드에서는 카드 음성이 나오지 않을 수 있습니다.
...
같은 v2.1.1의 빌더와 지원 PDF가 서로 맞지 않거나, PDF 표를 한자 골격과
...
문구, 사용한 v2.1.1, 운영체제와 멈춘 단계만 알려주세요.
...
책 본문, 페이지 이미지, 기본 덱이나 생성한 한자 덱 APKG, 개인 컬렉션과 전체
로컬 경로는 올리지 마세요.
