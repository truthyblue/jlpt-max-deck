...
| 원하는 구성 | 받을 파일 | 추가 준비물 |
| --- | --- | --- |
| 어휘·음성·실전 문제·참조표 | `JLPT-MAX-Deck-1.2.1.apkg` | Anki·AnkiMobile·AnkiDroid |
| 위 구성 + 일상무따 한자 | 기본 덱 + `JLPT-MAX-kanji-builder-1.2.1.zip` | 지원 PDF 2개, macOS 또는 Windows 컴퓨터 |

## 덱 구성

| 구성 | 노트 | 카드 | 미디어 |
| --- | ---: | ---: | ---: |
| 기본 덱: 어휘·음성·실전 문제·참조표 | 13,903 | 20,065 | 18,438 |
| 선택형 일상무따 한자 확장 | 2,337 | 2,337 | 14 |
| 두 APKG를 같은 컬렉션에 가져온 전체 | 16,240 | 22,402 | 18,452 |

기본 덱에는 어휘 6,018개, 실전 문제
7,876개, 참조표 9개와
단어·예문 음성이 들어 있습니다. 일상무따 한자 2,337개는 원할 때 아래 방법으로
...
182개를 추가했습니다. v1.1.1 이하 어느 기존 버전에서 업데이트하든 기존 노트
업데이트를 반드시 `항상`으로 선택하세요. 한자 확장도 v1.2.0 빌더로 다시 만들면
Windows Anki 야간 모드용 투명 SVG 렌더링과 새 일본어 글꼴, 최신 연결 어휘
표시가 함께 적용됩니다. 출발 버전별 이유는
[업데이트 가이드](https://truthyblue.github.io/jlpt-max-deck/update.html)에 정리했습니다.

v1.2.1은 뜻과 예문을 같은 순서로 맞추고, 단어 고저 악센트 선 끝에 다음 높낮이를
보여 주는 화살표를 추가했습니다. Mac Anki에서 카드를 넘길 때 일본어 글꼴이
번쩍 바뀌거나 글자가 움직이는 현상도 줄였고, 긴 후리가나와 한자 버튼도 더
안정적으로 보입니다. v1.2.0 사용자는 기존 노트 업데이트를
`새 버전일 때`로 가져오고 기존 한자 확장을 그대로 쓸 수 있습니다. v1.1.x와
v1.0.x에서 바로 업데이트한다면 v1.2.0의 새 필드와 템플릿도 적용해야 하므로
`항상`을 선택하세요.
...
1. [1권 공식 자료 페이지](https://www.gilbut.co.kr/book/view?bookcode=BN003617)와
   ...
   지원 소책자 PDF를 각각 받습니다.
2. 같은 v1.2.1의
   [JLPT-MAX-kanji-builder-1.2.1.zip](https://github.com/truthyblue/jlpt-max-deck/releases/download/v1.2.1/JLPT-MAX-kanji-builder-1.2.1.zip)을
   받아 압축을 완전히 풉니다.
...
5. 자동으로 열린 폴더의 `JLPT-MAX-kanji-addon-1.2.1.apkg`를
...
- 현재 자산과 검증 결과: [v1.2.1 릴리스 노트](docs/releases/v1.2.1.md)
